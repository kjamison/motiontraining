...
def pgrep_vlc():
  if sys.platform.startswith('win'):
    for p in psutil.process_iter():
      try:
        if re.search("VLC",p.name(),flags=re.IGNORECASE):
          return p.pid
      except:
        pass

  else:
    vlc_pidof = Popen("pgrep VLC", shell=True, stdout=PIPE).stdout
    vlc_pid=vlc_pidof.read().decode().replace("\n", "")
    return vlc_pid
  return ""
...
class VLCTelnet(object):
  ...
  @staticmethod
  def run_vlc(password="admin", port=4212):
    # Make sure VLC is open at the moment by using pidof. Otherwise raise VLCProcessError.
    vlc_pid = pgrep_vlc()
    if vlc_pid == "":
      if sys.platform.startswith('win'):
      ...
      if not os.path.exists(vlc_exe):
        raise VLCProcessError("Cannot find VLC executable: "+vlc_exe)
        
      vlc_args = [vlc_exe, "--extraintf","telnet","--telnet-port",str(port),"--telnet-password",str(password)]
      print " ".join(vlc_args)
      vlc_pid = Popen(vlc_args, stdin=None,stdout=None,stderr=None).pid
    return vlc_pid
  ...
  def pause(self):
    """Force pause.  Only toggle pause if currently playing"""
  ...
  def set_vdeinterlace(self, setto):
    ...
    command = 'vdeinterlace ' + str(setto)
    self.run_command(command)
  ...
  def set_vdeinterlace_mode(self, setto):
    ...
    command = 'vdeinterlace_mode ' + str(setto)
    self.run_command(command)
